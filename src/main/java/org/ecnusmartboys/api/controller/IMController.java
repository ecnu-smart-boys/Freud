...
public class IMController {
    ...
    @GetMapping("/userSig")
    public Response<String> userSig() {
        var userId = SecurityUtil.getCurrentUserId();
        var userSig = imConfig.getUserSig(String.valueOf(userId));
        return Response.ok("ok", userSig);
    }

    @PostMapping("/cb")
    @AnonymousAccess
    public Response<?> callback(IMCallbackParam param, @RequestBody String body, HttpServletRequest request) {
        try {
